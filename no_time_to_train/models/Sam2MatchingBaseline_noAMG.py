...
class Sam2MatchingBaselineNoAMG(nn.Module):
    ...
    def _compute_masks(
        ...
        # ---- DEBUG ONCE: print grid sizes to ensure alignment ----
        if not hasattr(self, "_dbg_printed"):
            print("[DEBUG] SAM embed grid =", self.predictor.sam_image_embedding_size,
                "| DINO grid =", backbone_features.size(2),
                "| DINO img_size (cfg) =", getattr(self, "encoder_img_size", "NA"))
        ...
        # 改成：
        import torch.nn.functional as F
        tgt = int(self.predictor.sam_image_embedding_size)  # e.g. 64
        h, w = backbone_features.size(2), backbone_features.size(3)
        if (h != tgt) or (w != tgt):
            backbone_features = F.interpolate(
                backbone_features, size=(tgt, tgt), mode="bilinear", align_corners=False
            )
