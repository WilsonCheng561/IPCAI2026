...
Author: Wenzheng Cheng (+ minimal modifications for test-only & distillation)
"""

import os, json, argparse, random, time
from pathlib import Path
from typing import List, Tuple, Dict, Optional
# --- put near other imports (safe to repeat) ---
import matplotlib
...
# --- 放在你的 import 区域靠前位置 ---
import sys
# 让 /home/wcheng31/sam2 成为 Python 包根（包含 sam2/ 与 configs/ 目录）
...
def _setup_hydra_configs():
    """
    使用 configs/sam2 作为 Hydra 的 config 根目录
    """
    if GlobalHydra.instance().is_initialized():
        GlobalHydra.instance().clear()
    initialize(config_path="configs/sam2", version_base="1.2")

# 优先用 backup 构建器（与参考一致），找不到再退回官方路径
try:
...
# OOD cholec80 默认测试清单目录（你的脚本输出）
CHOLEC80_TEST_ROOT = Path("/projects/surgical-video-digital-twin/Wenzheng/IPCAI/cholec80_test41-80")
...
def _plot_and_save_confmat(conf_mat: np.ndarray,
                           class_names: dict,
                           save_png: Path,
                           title: str = "",
                           normalize: bool = True):
    save_png.parent.mkdir(parents=True, exist_ok=True)
    cm = conf_mat.astype(np.float64)
    if normalize:
    ...
    classes = [class_names.get(i, f"class{i}") for i in range(conf_mat.shape[0])]
    ...
    for i in range(C):
        for j in range(C):
            val = cm[i, j] if normalize else conf_mat[i, j]
            txt = f"{val:.2f}" if normalize else f"{int(val)}"
            if C <= 20:
                ax.text(j, i, txt, ha="center", va="center",
                        fontsize=7, color=("white" if (normalize and val > 0.5) else "black"))

    plt.tight_layout()
    plt.savefig(str(save_png), bbox_inches="tight")
    plt.close(fig)

def _per_class_report_from_conf(conf_mat: np.ndarray, class_names: dict) -> pd.DataFrame:
    conf = conf_mat.astype(np.float64)
    tp = np.diag(conf)
    pred_pos = conf.sum(axis=0) + 1e-12
    true_pos = conf.sum(axis=1) + 1e-12
    prec = tp / pred_pos
    rec = tp / true_pos
...
def _class_stats(train_csv: Path, label_map_json: Path):
    ...
    # 兼容：若无 'tool' 列，尝试 'mapped_class'
    if "tool" not in df.columns and "mapped_class" in df.columns:
        df = df.rename(columns={"mapped_class": "tool"})
...
class FramePointDataset(Dataset):
    def __init__(self, manifest_csv: Path, label_map_json: Path, resize: Optional[int] = None):
        ...
        # 兼容：没有 'tool' 时，用 'mapped_class'
        if "tool" not in self.df.columns and "mapped_class" in self.df.columns:
            self.df = self.df.rename(columns={"mapped_class": "tool"})
    ...
    def __getitem__(self, idx: int):
        ...
        H, W = img.shape[:2]
        pts = json.loads(row["points_json"]) if ("points_json" in row and isinstance(row["points_json"], str) and row["points_json"].strip()) else []
        pts_out = []
        for p in pts:
            if len(p) < 2: continue
        ...
        return {
            ...
            "meta": {
                ...
                "tool": tool,
                "frame_abs_index": int(row.get("frame_abs_index", -1)) if "frame_abs_index" in row else -1,
                "frame_idx_in_clip": int(row.get("frame_idx_in_clip", -1)) if "frame_idx_in_clip" in row else -1,
                "clip_name": row.get("clip_name", "") if "clip_name" in row else "",
...
class FineTuneSam2Wrapper(nn.Module):
    def __init__(self, cfg: str, ckpt: str, device: str = "cuda", max_input_edge: int = 1536):
        ...
        cfg = os.path.basename(cfg) if isinstance(cfg, str) and cfg.endswith(".yaml") else cfg
    ...
    def set_trainable(self, which: str = "all"):
        ...
        mods = {
            "image": getattr(self.model, "image_encoder", None),
            "prompt": getattr(self.model, "prompt_encoder", None) or getattr(self.model, "sam_prompt_encoder", None),
            "mask": getattr(self.model, "mask_decoder", None) or getattr(self.model, "sam_mask_decoder", None),
            "all": self.model
        }
        if which == "none":
            return
        if which in mods and mods[which] is not None:
            for p in mods[which].parameters(): p.requires_grad_(True)
        else:
            for k in [s.strip() for s in which.split("+") if s.strip()]:
                m = mods.get(k, None)
                if m is not None:
    ...
    def _decode_mask(self, image_feat, image_pe, sparse_pe, dense_pe, high_res):
        ...
        out = md(image_embeddings=image_feat,
                 ...
                 repeat_image=True,
                 **({"high_res_features": hr} if hr is not None else {}))
    ...
    def forward(self,
        ...
        for i in range(B):
            ...
            is_bg = False
            if isinstance(targets, torch.Tensor) and i < targets.numel():
                is_bg = (int(targets[i].item()) == 0)

            # 先得到当前样本分辨率下的 mask_logits / mask
            if pts_np is None or len(pts_np) == 0:
                mask_logits = torch.full((1,1,Hf,Wf), fill_value=9.0, device=img_feat.device)
                mask = torch.sigmoid(mask_logits)
            else:
                mode = bg_mask_mode
                if is_bg and bg_mask_mode == "mix":
                ...
                if is_bg and mode == "global":
                    ...
                    coords = self._map_points_scale_xy(pts_np, sy, sx).to(img_feat.device)
                    if is_bg and mode == "pos":
                        ...
                        labels = torch.from_numpy(lab).long().unsqueeze(0).to(img_feat.device)
                    mask_logits = self._decode_mask(img_feat, img_pe, *self._encode_prompts(coords, labels), high_res)
                    if mask_logits.shape[-2:] != (Hf, Wf):
                        mask_logits = F.interpolate(mask_logits, size=(Hf, Wf), mode="bilinear", align_corners=False)
                    mask = torch.sigmoid(mask_logits)

            # 统一到 batch 参考分辨率
            if (Hf, Wf) != (H_ref, W_ref):
                mask = F.interpolate(mask, size=(H_ref, W_ref), mode="bilinear", align_corners=False)
                mask_logits = F.interpolate(mask_logits, size=(H_ref, W_ref), mode="bilinear", align_corners=False)
                img_feat_pool = F.interpolate(img_feat, size=(H_ref, W_ref), mode="bilinear", align_corners=False)
            else:
                img_feat_pool = img_feat

            feat = (img_feat_pool * mask).flatten(2).sum(dim=-1) / (mask.flatten(2).sum(dim=-1) + 1e-6)
            feats.append(feat.squeeze(0))

            if return_masks:
                masks_out.append(mask.detach())
            if return_mask_logits:
                logits_out.append(mask_logits)

            del img_t, img_feat, img_pe, high_res, mask, mask_logits

        feats = torch.stack(feats, dim=0)

        if return_masks and return_mask_logits:
            return feats, (torch.cat(masks_out, dim=0), torch.cat(logits_out, dim=0))
        elif return_masks:
            return feats, torch.cat(masks_out, dim=0)
        elif return_mask_logits:
            return feats, torch.cat(logits_out, dim=0)
        else:
            return feats
...
def dice_loss_with_probs(p: torch.Tensor, t: torch.Tensor, eps: float = 1e-6) -> torch.Tensor:
    p = p.float(); t = t.float()
    inter = (p * t).sum(dim=(1,2,3))
    denom = p.sum(dim=(1,2,3)) + t.sum(dim=(1,2,3)) + eps
    return (1.0 - (2.0 * inter / denom)).mean()

def mask_distill_loss(student_logits: torch.Tensor,
                      teacher_logits: torch.Tensor,
                      alpha: float = 1.0,
                      beta: float = 0.5) -> torch.Tensor:
    """
    alpha * BCEWithLogits(student, teacher_prob) + beta * Dice(student_prob, teacher_prob)
    """
    with torch.no_grad():
        t_prob = torch.sigmoid(teacher_logits)
    bce = F.binary_cross_entropy_with_logits(student_logits, t_prob)
    s_prob = torch.sigmoid(student_logits)
    dice = dice_loss_with_probs(s_prob, t_prob)
    return alpha * bce + beta * dice

class L2SP:
    """ \sum ||theta - theta0||^2 on selected modules """
    def __init__(self, model: nn.Module, modules=("model.image_encoder", "model.mask_decoder"), weight: float = 1e-4):
        self.weight = weight
        self.ref = {}
        for name, p in model.named_parameters():
            if any(name.startswith(m) for m in modules) and p.requires_grad and p.data.ndimension() > 0:
                self.ref[name] = p.data.clone().detach()
    def __call__(self, model: nn.Module) -> torch.Tensor:
        ...
        reg = None
        for name, p in model.named_parameters():
            if name in self.ref and p.requires_grad:
                term = (p - self.ref[name]).pow(2).sum()
                reg = term if reg is None else reg + term
        return self.weight * (reg if reg is not None else next(model.parameters()).new_zeros(()))
...
def train_one_epoch(extractor: nn.Module,
                    ...
                    # ------- 新增：蒸馏与正则 -------
                    teacher_extractor: Optional[nn.Module] = None,
                    distill_alpha: float = 1.0,
                    distill_beta: float = 0.5,
                    distill_weight: float = 1.0,
                    l2sp_reg: Optional["L2SP"] = None):
    ...
    for step, batch in enumerate(pbar, 1):
        ...
        with torch.cuda.amp.autocast(enabled=amp, dtype=amp_dtype):
            feats_s, s_mask_logits = extractor(
                imgs, pts, metas, targets=y,
                bg_mask_mode=bg_mask_mode, bg_mix_p=bg_mix_p,
                return_mask_logits=True
            )

            if do_mixup and feats_s.size(0) > 1:
                lam = np.random.beta(mixup_alpha, mixup_alpha)
                perm = torch.randperm(feats_s.size(0), device=feats_s.device)
                feats_mix = lam * feats_s + (1 - lam) * feats_s[perm]
                logits = head(feats_mix)
                logits = _apply_logit_adjust(logits, log_prior, logit_adjust_tau)
                cls_loss = lam * loss_fn(logits, y) + (1 - lam) * loss_fn(logits, y[perm])
            else:
                logits = head(feats_s)
                logits = _apply_logit_adjust(logits, log_prior, logit_adjust_tau)
            ...
            if teacher_extractor is not None:
                with torch.no_grad():
                    _, t_mask_logits = teacher_extractor(
                        imgs, pts, metas, targets=y,
                        bg_mask_mode=bg_mask_mode, bg_mix_p=bg_mix_p,
                        return_mask_logits=True
                    )
                distill = mask_distill_loss(s_mask_logits, t_mask_logits,
                                            alpha=distill_alpha, beta=distill_beta)
                total_loss = cls_loss + distill_weight * distill
            else:
                distill = torch.zeros((), device=logits.device)
                total_loss = cls_loss

            if l2sp_reg is not None:
                total_loss = total_loss + l2sp_reg(extractor)

        total_loss = total_loss / max(1, accum_steps)
        ...
        bs = y.size(0)
        running_loss += total_loss.item() * bs * max(1, accum_steps)
        n += bs
        pbar.set_postfix(loss=f"{(running_loss/max(1,n)):.4f}",
                         cls=f"{cls_loss.item():.4f}",
                         distill=f"{distill.item():.4f}")
        logger.write(f"epoch={epoch} step={step}/{len(loader)} "
                     f"train_loss={(running_loss/max(1,n)):.6f} "
                     f"cls={cls_loss.item():.6f} distill={distill.item():.6f}")
...
def evaluate(extractor: nn.Module, head: nn.Module, loader: DataLoader, device: str,
             ...
             bg_mask_mode: str = "mix", bg_mix_p: float = 0.5,
    ...
    if (n_classes is not None) and (len(y_true_all) > 0):
        ...
        # 保存 val/test 的混淆矩阵与每类报告
        if out_dir is not None:
            sub = "confmats" if split_name.lower()=="val" else "test_confmats"
            save_dir = Path(out_dir) / sub
            save_dir.mkdir(parents=True, exist_ok=True)
            cm_np = metrics["confusion"].numpy()
            title = f"{split_name.capitalize()} Confusion (epoch {epoch}) | macroF1={macro_f1:.3f} balAcc={bal_acc:.3f}"
            _plot_and_save_confmat(
                cm_np,
                class_names=(id2name or {}),
                save_png=save_dir / f"confmat_{split_name}_epoch{epoch:03d}.png",
                title=title,
                normalize=True
            )
            df_rep = _per_class_report_from_conf(cm_np, id2name or {})
            df_rep.to_csv(save_dir / f"report_{split_name}_epoch{epoch:03d}.csv", index=False)
            print(f"[{split_name.upper()}] Confusion & report saved to: {save_dir}")
...
def main():
    ...
    ap.add_argument("--trainable", type=str, default="all",
                    help="which parts of SAM2 to finetune: all|image|prompt|mask|image+mask|none")

    # resume (train or test)
    ap.add_argument("--resume", type=str, default=None,
                    help="path to finetune checkpoint (best_full_finetune.pt / full_epochxxx.pt) or legacy head-only checkpoint")
    ...
    # ---------- distill / reg ----------
    ap.add_argument("--exp-name", type=str, default="default", help="subdir under out-dir for training")
    ...
    ap.add_argument("--cholec80-root", type=str,
                    default="/projects/surgical-video-digital-twin/Wenzheng/IPCAI/cholec80_test41-80",
                    help="目录下需含 manifest.csv 与 label_map.json")
    # combined 测试默认指向 SMALLFILE_ROOT（与训练同源）
    # ap.add_argument("--combined-manifest", type=str, default=str(SMALLFILE_ROOT / "test_manifest_10.csv"))
    ap.add_argument("--combined-manifest", type=str, default=str(SMALLFILE_ROOT / "val_manifest_10.csv"))
    ap.add_argument("--combined-labelmap", type=str,  default=str(SMALLFILE_ROOT / "label_map.json"))
    # --- compatibility aliases for old flags ---
    ap.add_argument("--test-only", action="store_true",
                    help="Alias of --mode test (backward-compat).")
    ap.add_argument("--test-dataset", choices=["combined", "cholec80"], default=None,
                    help="Alias of --dataset (backward-compat).")

    # ===============================================

    args = ap.parse_args()
    # --- map legacy flags to new ones ---
    if getattr(args, "test_only", False):
        args.mode = "test"
    if getattr(args, "test_dataset", None) is not None:
    ...
    # ---------------- data / label map (train path) ----------------
    data_root = Path(args.data_root)
    manifest_path = Path(args.manifest) if args.manifest else (data_root / "manifest_10.csv")
    label_map_path = Path(args.label_map) if args.label_map else (data_root / "label_map.json")

    base_out = Path(args.out_dir) if args.out_dir else CKPT_ROOT
    out_dir = base_out / args.exp_name  # 训练输出 (<out-dir>/<exp-name>/...)
    ensure_dir(out_dir)
    ensure_dir(SMALLFILE_ROOT)

    # ---------------- TEST-ONLY 分支（不动训练路径/日志结构） ----------------
    if args.mode == "test":
        # 解析测试清单 & label_map（两数据集）
        if args.dataset == "combined":
            test_manifest_path = Path(args.combined_manifest)
            test_label_map_path = Path(args.combined_labelmap)
        else:
            ch_root = Path(args.cholec80_root)
            test_manifest_path = ch_root / "manifest.csv"
            test_label_map_path = ch_root / "label_map.json"

        if not test_manifest_path.exists():
            raise FileNotFoundError(f"Test manifest not found: {test_manifest_path}")
        if not test_label_map_path.exists():
            raise FileNotFoundError(f"Test label_map not found: {test_label_map_path}")

        # 用“测试集自己的 label_map”建立映射
        lm_test = load_label_map(test_label_map_path)
        tool2id = lm_test["tool_to_id"]
        ...
        # dataloader（保持和训练时的数据流一致）
        ds_eval = FramePointDataset(test_manifest_path, test_label_map_path, resize=args.resize)
        if len(ds_eval) == 0:
            raise RuntimeError("Empty test set after filtering. Check test manifest/points_json.")
        dl_eval = DataLoader(ds_eval, batch_size=args.batch_size, shuffle=False, num_workers=args.workers,
                             collate_fn=collate_varlen, pin_memory=True,
                             persistent_workers=(args.workers>0), prefetch_factor=2)

        # 构建提取器（推理态，冻结）
        extractor = FineTuneSam2Wrapper(args.sam2_cfg, args.sam2_ckpt,
                                        device=device, max_input_edge=args.max_input_edge).to(device)
        extractor.set_trainable("none")
        extractor = extractor.to(memory_format=torch.channels_last)

        # 探测 in_dim
        _probe = next(iter(dl_eval))
        with torch.no_grad():
            feat = extractor(_probe["images"][:1], _probe["points"][:1], _probe["meta"][:1])
        ...
        # 加载 checkpoint（优先全量 finetune；退化到 head-only 也可）
        if args.resume is None or not Path(args.resume).exists():
            raise FileNotFoundError("--resume is required for test mode and must exist.")
        ckpt = torch.load(args.resume, map_location="cpu")
        if isinstance(ckpt, dict) and ("sam2_state" in ckpt or "head_state" in ckpt):
            if "sam2_state" in ckpt:
                try:
                    extractor.load_state_dict(ckpt["sam2_state"], strict=False)
                except Exception:
                    pass  # 仅测试不强制
            state = ckpt.get("head_state", None)
        ...
        # logit-adjust 的先验：基于“测试清单 + 测试 label_map”
        t_counts, _, _, t_priors = _class_stats(test_manifest_path, test_label_map_path)
        log_prior = torch.log(torch.tensor(t_priors + 1e-12, dtype=torch.float32)) if t_priors is not None else None

        # ===== 保存路径（保持不变）：<out-dir>/<dataset>/unfreeze_sam2_MLP =====
        test_run_dir = (Path(args.out_dir) if args.out_dir else CKPT_ROOT) / args.dataset / ("unfreeze_sam2_MLP")
        ensure_dir(test_run_dir)

        # 简要打印
        print(f"Dataset: {args.dataset} ({str(test_manifest_path)})")
        print(f"Model  : SAM2({Path(args.sam2_cfg).name}, {Path(args.sam2_ckpt).name}) + Head({args.head}); ckpt={args.resume}")
        print(f"Save to: {str(test_run_dir)}")

        logger = TrainingLogger(test_run_dir / "test_log.txt")

        # 评测（沿用 evaluate，以保持与训练/验证同一统计口径）
        loss_fn = nn.CrossEntropyLoss()  # 报告用
        va_loss, va_acc, va_metrics = evaluate(
            extractor, head, dl_eval, device, epoch=0, logger=logger,
            ...
            out_dir=test_run_dir  # evaluate 仅在 val 保存混淆；这里传入不影响路径约定
        )
        logger.write(f"test_loss={va_loss:.6f} test_acc={va_acc:.6f} test_macroF1={va_metrics.get('macro_f1',0):.6f}")
        logger.close()
        return
    # ---------------- 结束 TEST 分支 ----------------
    ...
    ds_val   = FramePointDataset(val_csv,   label_map_path, resize=args.resize) if len(df_val)  else None
    ds_test  = FramePointDataset(test_csv,  label_map_path, resize=args.resize) if len(df_test) else None
    ...
    teacher_extractor = FineTuneSam2Wrapper(args.sam2_cfg, args.sam2_ckpt,
                                            device=device, max_input_edge=args.max_input_edge).to(device)
    teacher_extractor.set_trainable("none")
    teacher_extractor.eval()
    for p in teacher_extractor.parameters(): p.requires_grad_(False)

    if len(ds_train) == 0:
        raise RuntimeError("Empty training set after filtering.")
    _probe = next(iter(dl_train))
    with torch.no_grad():
        feat = extractor(_probe["images"][:1], _probe["points"][:1], _probe["meta"][:1])
    in_dim = int(feat.shape[-1])
    n_classes = len(tool2id)
    ...
    optim_groups = []
    if back_params:
        optim_groups.append({"params": back_params, "lr": args.lr_backbone, "weight_decay": args.wd_backbone})
    if head_params:
    ...
    if args.loss == "focal":
        ...
        if counts is None:
            ...
            cb_w = _cb_weights_from_counts(counts, beta=args.cb_beta).to(device)
            loss_fn = LDAMLoss(cls_num_list=counts, max_m=0.5, s=30, reweight=None)
    ...
    if args.sched == "cosine":
        sched = torch.optim.lr_scheduler.CosineAnnealingLR(
            opt, T_max=args.epochs, eta_min=min(args.lr_head, args.lr_backbone)*0.01
        )
    else:
        sched = None
    ...
    if args.resume is not None and Path(args.resume).exists():
        ckpt = torch.load(args.resume, map_location="cpu")
        if "sam2_state" in ckpt and "head_state" in ckpt:
            extractor.load_state_dict(ckpt["sam2_state"], strict=False)
            ...
            try:
                head.load_state_dict(state, strict=True)
                print(f"[RESUME] Loaded HEAD from legacy {args.resume}")
            except Exception as e:
                print(f"[RESUME] Legacy head strict load failed: {e}\nTrying non-strict...")
                head.load_state_dict(state, strict=False)
                print(f"[RESUME] Legacy head non-strict loaded.")

    logger = TrainingLogger(SMALLFILE_ROOT / args.exp_name / "train_finetune_log.txt")
    l2sp_reg = L2SP(model=extractor, modules=("model.image_encoder", "model.mask_decoder"),
                    weight=args.l2sp_weight) if args.l2sp_weight > 0 else None
    ...
    try:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            tr_loss = train_one_epoch(
                ...
                distill_alpha=args.distill_alpha, distill_beta=args.distill_beta,
            ...
            if improved:
                ...
                logger.write(f"epoch={epoch} SAVED best_full_finetune -> {best_path}")
            else:
                patience_left -= 1

            if (epoch % 5 == 0) or (epoch == args.epochs):
                ep_path = out_dir / f"full_epoch{epoch:03d}.pt"
                torch.save({
                    "sam2_state": extractor.state_dict(),
                    "head_state": head.state_dict(),
                    "in_dim": in_dim,
...
# python /home/wcheng31/sam2_classify/train_sam2_classify_finetune_distill.py \
...
#   --max-input-edge 1280 \
#   --amp \
#   --exp-name distill_maskcls_l \
#   --distill-alpha 1.0 --distill-beta 0.5 --distill-weight 1.0 \
#   --l2sp-weight 1e-4


# python /home/wcheng31/sam2_classify/train_sam2_classify_finetune_distill.py \
#   --epochs 30 --batch-size 64 --accum-steps 2 --seed 123 --patience 6 \
...
#   --max-input-edge 1024 \
#   --amp \
#   --exp-name distill_maskcls_t \
#   --distill-alpha 1.0 \
#   --distill-beta 0.5  \
#   --distill-weight 1.0 \
#   --l2sp-weight 1e-4

# 测 combined（与你训练同 split 的 test）：
...
#   --bg-mask-mode mix \
#   --logit-adjust 0.5 \
#   --resume /projects/surgical-video-digital-twin/pretrain_params/cwz/sam2_classifier/distill_maskcls_t/best_full_finetune.pt
#   --amp


# 测 cholec80 OOD（使用你脚本生成的 5 类 manifest/label_map）：
