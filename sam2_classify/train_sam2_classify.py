...
class FramePointDataset(Dataset):
    def __init__(self, manifest_csv: Path, label_map_json: Path,
        ...
        def has_points(s: str) -> bool:
            try:
                ...
                return False
        # 兼容 OOD 清单的列名（将 mapped_class 或 class 归一为 tool），不改变已有注释
    ...
    def _load_img(self, p: str):
        ...
        H0, W0 = img.shape[:2]          # 原图尺寸
        if self.resize and self.resize > 0:
            img = cv2.resize(img, (self.resize, self.resize), interpolation=cv2.INTER_AREA)
        return img, (H0, W0)            # 返回图像 + 原图尺寸


    def __getitem__(self, idx: int):
        row = self.df.iloc[idx]
        img, (H0, W0) = self._load_img(row["image_path"])  # 现在拿得到原图尺寸
        H1, W1 = img.shape[:2]                             # 实际喂入模型的尺寸（可能与原图不同）

        # 解析点（原始 JSON 坐标是基于原图 H0,W0）
        ...
        # 如果做了 resize，就把点从 (W0,H0) 仿射到 (W1,H1)
        if (H0 != H1) or (W0 != W1):
        ...
        # 背景策略（保留你原逻辑，按需要改 self.bg_mask_mode）
        if int(tool_id) == 0:
...
class Sam2OfficialWrapper(nn.Module):
    ...
    def _get_image_embed(self, img_t: torch.Tensor):
        ...
        if isinstance(out, dict) and ("vision_features" in out):
            ...
            # ---- debug shapes for classifier tap points ----
            dbg_img = tuple(img_feat.shape)  # (B, C, Hf, Wf)
            dbg_pe  = None if img_pe is None else tuple(img_pe.shape)
            dbg_hr0 = dbg_hr1 = None
            if isinstance(high_res, tuple) and len(high_res) == 2:
                dbg_hr0 = tuple(high_res[0].shape)
                dbg_hr1 = tuple(high_res[1].shape)
            # print(f"[DBG] image_feat={dbg_img} | image_pe={dbg_pe} | high_res0={dbg_hr0} | high_res1={dbg_hr1}")
    ...
    def forward(
        ...
        """
        return_4d=False: 返回 [B,C] 的 masked-GAP 特征（原有行为）
        return_4d=True : 返回 (img_feat[B,C,Ht,Wt], mask[B,1,Ht,Wt], img_pe[B,C,Ht,Wt])
                        批内自动空间对齐，避免 cat 尺寸不一致的异常。
        """
        pooled_vecs = []
        per_sample  = []  # [(img_feat[1,C,h,w], mask[1,1,h,w], img_pe_or_none[1,C,h,w]), ...]
        ...
        for i, (img_bgr, pts_np) in enumerate(zip(images_bgr, points_list)):
            ...
            if not return_4d:
                # -------- masked GAP -> [C] --------
                vec = (img_feat * mask).flatten(2).sum(dim=-1) / (mask.flatten(2).sum(dim=-1) + 1e-6)
                pooled_vecs.append(vec.squeeze(0))
            else:
                per_sample.append((img_feat, mask, img_pe))

        if not return_4d:
            return torch.stack(pooled_vecs, dim=0)  # [B,C]

        # -------------- 批内尺寸对齐 + 打印 --------------
        hw_list = [tuple(t[0].shape[-2:]) for t in per_sample]  # 每个样本的 (h, w)
        print("[DBG] per-sample HxW:", ", ".join([f"{h}x{w}" for (h, w) in hw_list]))
        tgt_h = max(h for (h, w) in hw_list)
        tgt_w = max(w for (h, w) in hw_list)
        print(f"[DBG] target HxW -> {tgt_h}x{tgt_w}")

        def _resize(x: Optional[torch.Tensor]) -> Optional[torch.Tensor]:
            if x is None:
                return None
        ...
        img_feats_b, masks_b, img_pes_b = [], [], []
        for (img_feat, mask, img_pe) in per_sample:
            img_feats_b.append(_resize(img_feat))
            masks_b.append(_resize(mask))
            img_pes_b.append(_resize(img_pe) if (img_pe is not None) else None)

        # 若存在 None 的 pe，用零张量补齐再 stack
        if any(pe is None for pe in img_pes_b):
            for i in range(len(img_pes_b)):
                if img_pes_b[i] is None:
                    img_pes_b[i] = torch.zeros_like(img_feats_b[i])

        img_feat_b = torch.cat(img_feats_b, dim=0)  # [B,C,Ht,Wt]
        mask_b     = torch.cat(masks_b, dim=0)      # [B,1,Ht,Wt]
        img_pe_b   = torch.cat(img_pes_b, dim=0)    # [B,C,Ht,Wt]
        # print(f"[DBG] stacked image_feat={tuple(img_feat_b.shape)} mask={tuple(mask_b.shape)} pe={tuple(img_pe_b.shape)}")

        return (img_feat_b, mask_b, img_pe_b)
...
def evaluate_and_dump(extractor: nn.Module,
    ...
    评测并将结果落盘：
      ...
      - predictions.csv （逐样本）
      - summary.txt （总体与分类别acc）
    保存目录：save_dir（外部传入；保持你当前使用的 CKPT_ROOT/eval/...）
    """
    ...
    per_acc = (cm.diagonal() / totals).tolist()
    print("[TEST per-class acc]")
    for cid in range(n_classes):
        print(f"  {cid:2d} {id2name.get(cid, f'class{cid}'):>16s}: acc={per_acc[cid]:.4f} (n={int(totals[cid])})")
    ...
    try:
        ...
        ax.set_ylabel("True")
        ax.set_xticks(range(n_classes)); ax.set_xticklabels([id2name.get(i,str(i)) for i in range(n_classes)], rotation=45, ha='right')
        ax.set_yticks(range(n_classes)); ax.set_yticklabels([id2name.get(i,str(i)) for i in range(n_classes)])
        for (i,j), v in np.ndenumerate(cm):
            ax.text(j, i, int(v), ha='center', va='center', fontsize=8)
        fig.tight_layout()
        (save_dir / "confusion_matrix.png").parent.mkdir(parents=True, exist_ok=True)
        fig.savefig(save_dir / "confusion_matrix.png", dpi=180)
        plt.close(fig)
    except Exception as e:
        print(f"[WARN] Could not save confusion matrix figure: {e}")
    ...
    df = pd.DataFrame({
        ...
        "true_id": y_true, "true_name": [id2name.get(int(t), str(int(t))) for t in y_true],
        "pred_id": y_pred, "pred_name": [id2name.get(int(p), str(int(p))) for p in y_pred],
        "pred_conf": top1_prob
    })
    df.to_csv(save_dir / "predictions.csv", index=False)

    # 混淆矩阵 CSV
    pd.DataFrame(cm, columns=[id2name.get(i,str(i)) for i in range(n_classes)],
    ...
    with open(save_dir / "summary.txt", "w") as f:
        ...
        for cid in range(n_classes):
            f.write(f"cls[{cid}][{id2name.get(cid,str(cid))}]_acc={per_acc[cid]:.6f} n={int(totals[cid])}\n")
...
def main():
    ...
    ap.add_argument("--mode", choices=["train","test"], default="train",
                    help="train: 训练并可选测试；test: 只测试")
    ap.add_argument("--test-only", action="store_true", help="与 --mode test 等价；只做评测不训练")
    ap.add_argument("--test-dataset", choices=["combined","cholec80"], default="combined",
                    help="测试数据集选择")
    ...
    ap.add_argument("--save-root", type=str, default=str(CKPT_ROOT / "eval"),
                    help="测试结果保存根目录（保持不变：<CKPT_ROOT>/eval/...）")
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("CUDA is required but not available. Please run on a GPU-enabled environment.")
    device = "cuda"
    ...
    # 加载（训练 split 的）label_map 仅用于训练模式
    ...
    # datasets & loaders（训练/验证/测试 split）
    ...
    # ============================== TEST 模式（最小修复版） ==============================
        # ======= test-only 早返回（与 distill 脚本一致的行为）=======
    if args.test_only or args.mode == "test":
        # 1) 构造测试清单 & 标签映射路径
        if args.test_dataset == "combined":
        ...
        assert "background" in tool2id_eval and tool2id_eval["background"] == 0, \
            f"label_map.json must include 'background': 0, current={tool2id_eval.get('background')}"
        ...
        # 5) 先验（如果你训练阶段有保存/可复用）
        # 这里沿用训练集的 log_prior 近似：用测试集标签分布会泄露；因此默认不变更。
        # 若你有训练期统计，可以在此处读入替换。
        counts, _, _, priors = _class_stats(test_manifest_path, test_labelmap_path)  # 可选：仅用于打印
        log_prior_eval = None  # 默认不做先验调整；若想启用，请改为：torch.log(torch.tensor(priors+1e-12)).float()

        # 6) 保存目录：保持不变 <CKPT_ROOT>/eval/<dataset>/froze_sam2_MLP
        save_dir = Path(args.save_root) / args.test_dataset / "froze_sam2_MLP"
        save_dir.mkdir(parents=True, exist_ok=True)

        print(f"Dataset : {args.test_dataset} ({test_manifest_path})")
        ...
        _ = evaluate_and_dump(
            ...
            save_dir=save_dir,
            log_prior=log_prior_eval, logit_adjust_tau=args.logit_adjust
        )
        return
...
#   --backend official \
#   --epochs 20 --batch-size 1024 --seed 123 --patience 5 \
...
# /projects/surgical-video-digital-twin/pretrain_params/cwz/sam2_classifier/vis_test
