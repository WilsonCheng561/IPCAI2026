...
class Sam2OfficialWrapper(nn.Module):
    ...
    @torch.no_grad()
    def _align_batch_4d(self, feats_list, interp_mode: str = "bilinear"):
        """
        feats_list: List[Tuple[img_feat(1,C,h,w), mask(1,1,h,w), img_pe(1,C,h,w) or None]]
        返回: (img_feat[B,C,Ht,Wt], mask[B,1,Ht,Wt], img_pe[B,C,Ht,Wt])
        逻辑：对齐到本 batch 最大的 Ht×Wt，然后再 cat。
        """
        # 收集每个样本的 (h, w)，取批内最大作为目标
        sizes = [t[0].shape[-2:] for t in feats_list]
        tgt_h = max(h for h, _ in sizes)
        tgt_w = max(w for _, w in sizes)

        def _resize(x, size_hw):
            if x is None:
                return None
            if x.shape[-2:] == size_hw:
                return x
            return F.interpolate(x, size=size_hw, mode=interp_mode, align_corners=False)

        img_feats, masks, pos = [], [], []
        for (ff, mm, pp) in feats_list:
            ff_r = _resize(ff, (tgt_h, tgt_w))
            mm_r = _resize(mm, (tgt_h, tgt_w))
            if pp is None:
                pp_r = torch.zeros_like(ff_r)
            else:
                pp_r = _resize(pp, (tgt_h, tgt_w))
            img_feats.append(ff_r)
            masks.append(mm_r)
            pos.append(pp_r)

        uniq_h = sorted({h for h, _ in sizes})
        uniq_w = sorted({w for _, w in sizes})
        # print(f"[ALIGN] unique H={uniq_h} unique W={uniq_w} -> target {tgt_h}x{tgt_w}") #unique H=[20, 28, 32, 44, 48] unique W=[40, 52, 60, 80] -> target 48x80
        img_feat_b = torch.cat(img_feats, dim=0)  # [B,C,Ht,Wt]
        mask_b     = torch.cat(masks,    dim=0)  # [B,1,Ht,Wt]
        pos_b      = torch.cat(pos,      dim=0)  # [B,C,Ht,Wt]
        # print(f"[ALIGN] stacked image_feat={tuple(img_feat_b.shape)} mask={tuple(mask_b.shape)} pos={tuple(pos_b.shape)}") #[ALIGN] stacked image_feat=(256, 256, 48, 80) mask=(256, 1, 48, 80) pos=(256, 256, 48, 80)
    ...
    def forward(
        ...
        """
        return_4d=False: 返回 [B, C]（masked GAP），保持你原来的分类头输入。
        return_4d=True : 返回对齐后的三件套 ([B,C,Ht,Wt], [B,1,Ht,Wt], [B,C,Ht,Wt])，给 ViT 头用。
        """
        feats_triplets = []  # 每个元素: (img_feat(1,C,h,w), mask(1,1,h,w), img_pe(1,C,h,w)|None)
        ...
        for i, (img_bgr, pts_np) in enumerate(zip(images_bgr, points_list)):
            ...
            if cached is None:
                ...
                img_feat, img_pe, high_res = self._get_image_embed(img_t)  # [1,C,h,w], [1,C,h,w]|None, (..)|None
        ...
        if return_4d:
            ...
            return img_feat_b, mask_b, pos_b  # [B,C,Ht,Wt], [B,1,Ht,Wt], [B,C,Ht,Wt]

        # 默认：masked GAP，输出 [B, C]
        pooled = []
        for (img_feat, mask, _) in feats_triplets:
            feat = (img_feat * mask).flatten(2).sum(dim=-1) / (mask.flatten(2).sum(dim=-1) + 1e-6)  # [1,C]
...
class ViTTokenHead(nn.Module):
    ...
    def __init__(self, in_dim: int, n_classes: int, num_layers: int = 2, num_heads: int = 4, mlp_ratio: float = 4.0, p_drop: float = 0.1):
        super().__init__()
    ...
    def forward(self, img_feat: torch.Tensor, mask: Optional[torch.Tensor] = None, pos: Optional[torch.Tensor] = None):
        ...
        key_padding = None
        if mask is not None:
            keep = (mask > 0.5).flatten(1)           # [B,N] True=keep
            key_padding = (~keep).bool()             # True=pad屏蔽
        ...
        x = self.enc(x, src_key_padding_mask=key_padding)        # [B,1+N,C]
        cls_out = self.norm(x[:,0])                              # [B,C]
...
def train_one_epoch(extractor: nn.Module, head: nn.Module, loader: DataLoader, device: str,
    ...
    for step, batch in enumerate(pbar, 1):
        ...
        with torch.no_grad():
            if head_type == "vit":
                img_feat, mask, img_pe = extractor(imgs, pts, metas, return_4d=True)  # [B,C,H,W],[B,1,H,W],[B,C,H,W] or None
                feats_for_head = (img_feat, mask, img_pe)
            else:
                vecs = extractor(imgs, pts, metas, return_4d=False)  # [B,C]
                feats_for_head = vecs
...
def main():
    ...
    # 默认用 tiny，你可以改成 large；脚本会自动探测 C,H,W
    ...
    ap.add_argument("--vit-drop",   type=float, default=0.1)
    ...
    dl_train = DataLoader(ds_train, batch_size=args.batch_size, shuffle=True,  num_workers=args.workers, collate_fn=collate_varlen, pin_memory=True)
    dl_val   = DataLoader(ds_val,   batch_size=args.batch_size, shuffle=False, num_workers=args.workers, collate_fn=collate_varlen, pin_memory=True) if ds_val else None
    dl_test  = DataLoader(ds_test,  batch_size=args.batch_size, shuffle=False, num_workers=args.workers, collate_fn=collate_varlen, pin_memory=True) if ds_test else None
    ...
    if (args.balance in ("auto","sampler")) and (counts is not None):
        ...
        if use_sampler:
            ...
            dl_train = DataLoader(ds_train, batch_size=args.batch_size, shuffle=False, num_workers=args.workers,
    ...
    with torch.no_grad():
        if args.head == "vit":
            ...
            C = int(img_feat.shape[1]); Hf = int(img_feat.shape[2]); Wf = int(img_feat.shape[3])
            in_dim = C
    ...
    if args.head == "vit":
        head = ViTTokenHead(in_dim=in_dim, n_classes=n_classes,
                            ...
                            mlp_ratio=args.vit_mlp_ratio, p_drop=args.vit_drop).to(device)
...
#   --head vit --vit-layers 2 --vit-heads 4 --vit-drop 0.1
