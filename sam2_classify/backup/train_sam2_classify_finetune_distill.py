...
def _setup_hydra_configs():
    ...
    # 这里的路径相对于 /home/wcheng31/sam2（我们已加入 sys.path）
...
def _plot_and_save_confmat(conf_mat: np.ndarray,
                           ...
                           normalize: bool = True):
    """
    conf_mat: (C,C) ndarray of ints (rows = true, cols = pred)
    ...
    # optional: annotate values (sparse to避免过密)
    C = conf_mat.shape[0]
    for i in range(C):
        for j in range(C):
...
def _per_class_report_from_conf(conf_mat: np.ndarray, class_names: dict) -> pd.DataFrame:
    """
    返回每类 precision/recall/F1/support 的 DataFrame
    """
...
class FineTuneSam2Wrapper(nn.Module):
    ...
    def forward(self,
        ...
        """
        - 先把 mask_logits 对齐到当前样本的 image_feat(Hf,Wf)
        - 再把 mask / mask_logits 统一到本 batch 的参考尺寸(ref_hw)
        - img_feat 也在加权池化前对齐到 ref_hw，避免乘法尺寸不匹配
        """
        feats = []
        masks_out = []
        logits_out = []
        ...
        for i in range(B):
            ...
            # ---- 先生成当前样本的 mask_logits / mask（原生分辨率）----
            if pts_np is None or len(pts_np) == 0:
                mask_logits = torch.full((1,1,Hf,Wf), fill_value=9.0, device=img_feat.device)  # sigmoid≈1
                mask = torch.sigmoid(mask_logits)
            else:
                mode = bg_mask_mode
                if is_bg and bg_mask_mode == "mix":
                ...
                if is_bg and mode == "global":
                    ...
                    # ★ 关键补丁：如果 mask_decoder 输出的尺寸 != (Hf,Wf)，先对齐到特征尺寸
    ...
    def __call__(self, model: nn.Module) -> torch.Tensor:
        ...
        for name, p in model.named_parameters():
            if name in self.ref and p.requires_grad:
                reg = reg + (p - self.ref[name]).pow(2).sum()
...
def train_one_epoch(extractor: nn.Module,
    ...
    for step, batch in enumerate(pbar, 1):
        ...
        bs = y.size(0)
        running_loss += total_loss.item() * bs * max(1, accum_steps)  # 还原
        n += bs
...
def evaluate(extractor: nn.Module, head: nn.Module, loader: DataLoader, device: str,
    ...
    """
    新增参数:
      - out_dir: 根输出目录；若 split_name=='val' 则在 out_dir/'confmats' 下保存 PNG/CSV
    """
    ...
    if (n_classes is not None) and (len(y_true_all) > 0):
        ...
        # ------- save confusion matrix & per-class CSV for VAL only -------
        if split_name.lower() == "val" and out_dir is not None:
            save_dir = Path(out_dir) / "confmats"
            save_dir.mkdir(parents=True, exist_ok=True)
            # PNG
            cm_np = metrics["confusion"].numpy()
            title = f"Val Confusion (epoch {epoch}) | macroF1={macro_f1:.3f} balAcc={bal_acc:.3f}"
            _plot_and_save_confmat(
                cm_np,
                class_names=(id2name or {}),
                save_png=save_dir / f"confmat_val_epoch{epoch:03d}.png",
                title=title,
                normalize=True
            )
            # CSV（每类指标）
            df_rep = _per_class_report_from_conf(cm_np, id2name or {})
            df_rep.to_csv(save_dir / f"report_val_epoch{epoch:03d}.csv", index=False)
...
def main():
    ...
    # ------------------ 新增：实验名 / 蒸馏 / L2SP ------------------
    ap.add_argument("--exp-name", type=str, default="default",
                    help="子目录名：<out-dir>/<exp-name>")
    ...
    ap.add_argument("--distill-weight", type=float, default=1.0,
                    help="mask蒸馏整体系数（乘在alpha,beta项外层）")
    ...
    # === 统一输出：<out-dir>/<exp-name>/...
    base_out = Path(args.out_dir) if args.out_dir else CKPT_ROOT
    out_dir = base_out / args.exp_name
    ensure_dir(out_dir)

    # === 日志：也放进 exp-name 子目录
    ensure_dir(SMALLFILE_ROOT / args.exp_name)
    log_file = SMALLFILE_ROOT / args.exp_name / "train_finetune_log.txt"
    ...
    for p in teacher_extractor.parameters():
        p.requires_grad_(False)
    ...
    # L2-SP 正则器（仅约束image_encoder与mask_decoder，避免把分类头拉回初始化）
    l2sp_reg = None
    if args.l2sp_weight > 0:
        l2sp_reg = L2SP(
            model=extractor,
            modules=("model.image_encoder", "model.mask_decoder"),
            weight=args.l2sp_weight
        )
