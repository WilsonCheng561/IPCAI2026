...
class Sam2OfficialWrapper(nn.Module):
    ...
    def _align_batch_4d(self, feats_triplets):
        """
        feats_triplets: List[Tuple[img_feat(1,C,h,w), mask(1,1,h,w), img_pe(1,C,h,w)|None]]
        返回下采样到本 batch **最小** 尺寸后的三件套:
          img_feat_b: [B,C,Hmin,Wmin]
          mask_b    : [B,1,Hmin,Wmin]
          pos_b     : [B,C,Hmin,Wmin]
        """
        sizes = [t[0].shape[-2:] for t in feats_triplets]
        tgt_h = min(h for h, _ in sizes)   # <<< CHANGED: 对齐到最小
        tgt_w = min(w for _, w in sizes)   # <<< CHANGED

        def _pool_to(x, size_hw):
            if x is None: return None
            if x.shape[-2:] == size_hw: return x
            return F.adaptive_avg_pool2d(x, size_hw)  # <<< CHANGED: 下采样（自适应平均）
...
class ViTTokenHead(nn.Module):
    def __init__(self, in_dim: int, n_classes: int,
                 ...
                 mlp_ratio: float = 4.0, p_drop: float = 0.05,     # 默认 0.05
                 max_tokens: int = 512):                           # <<< CHANGED: 新增参数
        super().__init__()
        self.max_tokens = int(max_tokens)                         # <<< CHANGED
...
def evaluate(extractor: nn.Module, head: nn.Module, loader: DataLoader, device: str,
    ...
    torch.cuda.empty_cache()  # <<< CHANGED: eval 前清理显存，缓解碎片
    head.eval()
    total_loss, n = 0.0, 0
    correct = 0
    printed_shapes = False
...
def main():
    ...
    ap.add_argument("--vit-drop",   type=float, default=0.05)  # 默认 0.05
    ap.add_argument("--vit-mlp-ratio", type=float, default=4.0)
    ap.add_argument("--vit-max-tokens", type=int, default=512, help="cap tokens per sample before Transformer")  # <<< CHANGED
    ...
    # <<< CHANGED: eval/test 独立 batch size >>>
    ...
    train_bs = args.batch_size                                            # <<< CHANGED
    ...
    if args.head == "vit":
        head = ViTTokenHead(in_dim=in_dim, n_classes=n_classes,
                            ...
                            p_drop=args.vit_drop, max_tokens=args.vit_max_tokens).to(device)  # <<< CHANGED: 传入 max_tokens
