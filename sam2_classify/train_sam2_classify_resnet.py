...
class FramePointDataset(Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        if (H0 != H1) or (W0 != W1):
            sy = float(H1) / max(1.0, float(H0)); sx = float(W1) / max(1.0, float(W0))
            if pts_np.size > 0: pts_np[:, 0] *= sx; pts_np[:, 1] *= sy
...
class ResNetExtractor(nn.Module):
    """Convert BGR np.ndarray list -> normalized tensor batch [B,3,H,W]."""
...
def evaluate(extractor: nn.Module, head: nn.Module, loader: DataLoader, device: str,
             ...
             save_dir: Optional[Path] = None):  # ==== ADDED
    ...
    if (cm is not None) and (id2name is not None):
        ...
        # ==== ADDED: 可将保存目录切到 save_dir ====
        out_dir = Path(save_dir) if save_dir is not None else SMALLFILE_ROOT
        ensure_dir(out_dir)
        save_path = out_dir / f"cm_{split_name}_epoch{epoch:03d}.png"
...
def main():
    ...
    if args.mode == "test":
        ...
        if (args.resume is None) or (not Path(args.resume).exists()):
            raise FileNotFoundError("--resume is required in test mode and must point to a checkpoint.")
        ...
        if args.dataset == "combined":
            ...
            mani = Path(args.cholec80_manifest)
            lbl  = Path(args.cholec80_labelmap)

        ds_test = FramePointDataset(mani, lbl, resize=args.resize)
        dl_test = DataLoader(ds_test, batch_size=(args.test_batch_size or args.batch_size),
                             shuffle=False, num_workers=args.workers,
                             collate_fn=collate_varlen, pin_memory=True)

        # save dir
        save_root = Path(args.save_root)
        # save_dir  = save_root / f"{Path(args.resume).stem}_{args.dataset}"
        save_dir = Path(args.save_root) / args.dataset / ("resnet101")
        ensure_dir(save_dir)

        # three English prints
        print(f"Dataset: {args.dataset} -> {mani}")
        print(f"Model: {args.arch} (pretrained={bool(args.pretrained)})")
        print(f"Save to: {save_dir}")

        loss_fn = nn.CrossEntropyLoss()
        _ = evaluate(extractor, model, dl_test, device, epoch=0,
                     logger=TrainingLogger(save_dir / "eval_log.txt"),
                     loss_fn=loss_fn, split_name="test",
                     n_classes=n_classes, id2name=id2name,
                     log_prior=None, logit_adjust_tau=0.0,
                     head_type="resnet", save_dir=save_dir)
        return
    # ==== ADDED END ====

    # datasets & loaders (train/val/test)
    df_val = pd.read_csv(val_csv)
    df_test = pd.read_csv(test_csv) if test_csv.exists() else pd.DataFrame(columns=df_val.columns)
    ...
    try:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            va_loss, va_acc = evaluate(extractor, model, dl_val, device, epoch, logger, loss_fn=loss_fn,
                                       ...
                                       log_prior=log_prior, logit_adjust_tau=args.logit_adjust, head_type="resnet") if dl_val else (0.0, 0.0)
...
# combined 测试
# python /home/wcheng31/sam2_classify/train_sam2_classify_resnet.py \
#   --mode test --dataset combined \
#   --arch resnet101 --input-size 224 --pretrained \
#   --test-batch-size 64 \
#   --resume /projects/surgical-video-digital-twin/pretrain_params/cwz/sam2_classifier/resnet101/best_resnet101.pt \
#   --save-root /projects/surgical-video-digital-twin/pretrain_params/cwz/sam2_classifier/eval_resnet

# cholec80 测试
# python /home/wcheng31/sam2_classify/train_sam2_classify_resnet.py \
#   --mode test --dataset cholec80 \
#   --arch resnet101 --input-size 224 --pretrained \
#   --test-batch-size 64 \
#   --resume /projects/surgical-video-digital-twin/pretrain_params/cwz/sam2_classifier/resnet101/best_resnet101.pt \
#   --save-root /projects/surgical-video-digital-twin/pretrain_params/cwz/sam2_classifier/eval_resnet \
#   --cholec80-manifest /projects/surgical-video-digital-twin/Wenzheng/IPCAI/cholec80_test41-80/manifest.csv \
#   --cholec80-labelmap  /projects/surgical-video-digital-twin/Wenzheng/IPCAI/cholec80_test41-80/label_map.json
